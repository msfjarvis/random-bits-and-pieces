...
def analyze_translations(source_file, tolerance, remove_incomplete):
    ...
    for found_file in search_files(file_name=file_to_process):
        ...
        if percentage < tolerance:
            print(
                "%s is not in sync with source files, missing %s strings"
...
def main():
    ...
    parser.add_argument(
        "-s",
        "--source",
        help="Source file to compare against; it's assumed "
    ...
    parser.add_argument(
        "-t",
        "--tolerance",
        help="Percentage of missing strings that are "
    ...
    parser.add_argument(
        "-r",
        "--remove",
        help="Remove localizations not in the tolerance range",
        action="store_true",
    )
